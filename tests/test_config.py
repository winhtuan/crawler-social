...
def test_proxy_parse_no_port():
    p = Proxy.from_url("http://127.0.0.1")
    assert p.server == "http://127.0.0.1"

def test_proxy_parse_empty():
    assert Proxy.from_url(None) is None
    assert Proxy.from_url("") is None
